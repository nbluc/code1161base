...
def three_counter(input_list=[1, 4, 3, 5, 7, 1, 3, 2, 3, 3, 5, 3, 7]):
    ...
    """
    countfunction = input_list.count(3)
    return int(countfunction)
    print (countfunction)
...
def put_behind_bars(input_string="very naughty boy"):
    ...
    """
    pipe = "|"
    return pipe + (pipe.join(input_string)) + pipe
    print (pipe + (pipe.join(input_string)) + pipe)
...
def pet_filter(letter="a"):
    ...
    res = [k for k in pets if letter in k]
    return res
